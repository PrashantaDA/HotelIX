            ...
            >
              ...
              <div className="flex flex-col md:flex-row gap-8 items-center">
                ...
                <div className="flex-1 text-center md:text-left">
                  ...
                  <p className="text-normal/90 text-lg leading-relaxed mb-6 italic">
                    "{testimonials[current].text}"
                  </p>

                  <div>
                    <h4 className="text-normal font-semibold text-lg">{testimonials[current].name}</h4>
                    <p className="text-secondary text-sm">{testimonials[current].country} · {testimonials[current].date}</p>
                  </div>
                </div>
              </div>
            </motion.div>
            ...
            {testimonials.map((_, i) => (
              <button
                ...
                className={`w-2 h-2 rounded-full transition-all duration-300 ${
                  i === current ? "w-8 bg-primary" : "bg-secondary/30 hover:bg-secondary/50"
                }`}
              />
            ))}
          </div>
